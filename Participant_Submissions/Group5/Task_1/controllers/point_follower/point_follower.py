...
def point_follower(current,goal):
    ...
    # atan2(yg-yc, xg-xc) = angle

    if(abs(x-goal[0]) < 0.03 and abs(y-goal[1]) < 0.03 and abs(theta-goal[2]) > 0.01):
        leftSpeed = -1.0  
        rightSpeed = 1.0
    else:
        leftSpeed = 6.0 
...
if __name__=="__main__":
    ...
    # ------------------------------------------------------------------------------
    goal = [[4.0, -4.0, 1.57],[4.0, 4.0, 3.14],[-4.0, 4.0, -1.57],[-4.0, -4.0, -1.57]]
    goal_elem = goal[0]
    i = 0
    while robot.step(timestep) != -1:
        ...
        if(abs(current[0]-goal[i][0]) < 0.05 and abs(current[1]-goal[i][1]) < 0.05 and abs(current[2]-goal[i][2]) < 0.01):
            i = i+1
        
        if(i == 3 and abs(current[0]-goal[i][0]) < 0.2 and abs(current[1]-goal[i][1]) < 0.2):
            leftSpeed, rightSpeed = 0,0
        else:
            leftSpeed,rightSpeed = point_follower(current,goal[i])
        ## ------------------------------------------------------------------------------
