...
if __name__=="__main__":
    ...
    while robot.step(timestep) != -1:
        ...
        x=current[0]+4.9
        y=(-5*math.sin(2*x/math.pi))+0.1
        theta=math.atan((-10/math.pi)*math.cos(2*x/math.pi))
        #(-1.39)*math.cos(2*x/math.pi)
        #(-1/18)*math.cos(2*x/math.pi)
        #math.asin(dy/(math.sqrt(dx*dx+dy*dy)))
        goal = [x,-5*math.sin(2*x/math.pi),theta] # initial goal to initialize goal array
        ## ------------------------------------------------------------------------------
        #Edit here 
        ...
        if (abs(current[0]-5) < 0.05) and (abs(current[1]-0) < 0.05):
